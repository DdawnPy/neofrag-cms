		...
		$news = $this	->table
						->add_columns(array(
							...
							array(
								'content' => array(
									function($data){
										return $this->is_authorized('edit_news') ? button_edit('admin/news/'.$data['news_id'].'/'.url_title($data['title']).'.html') : NULL;
									},
									function($data){
		...
		$categories = $this	->table
							->add_columns(array(
								...
								array(
									'content' => array(
										function($data){
											return $this->is_authorized('edit_news_category') ? button_edit('admin/news/categories/'.$data['category_id'].'/'.$data['name'].'.html') : NULL;
										},
										function($data){
